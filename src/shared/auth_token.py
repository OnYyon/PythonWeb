...
class JwtTokenManager:
    def __init__(
        ...
        self.access_minutes = access_minutes or int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES", "30"))
        self.refresh_days = refresh_days or int(os.getenv("REFRESH_TOKEN_EXPIRE_DAYS", "14"))

    def create_access_token(self, claims: dict[str, Any]) -> str:
        return self._encode(claims=claims, token_type="access", expires=timedelta(minutes=self.access_minutes))

    def create_refresh_token(self, claims: dict[str, Any]) -> str:
        return self._encode(claims=claims, token_type="refresh", expires=timedelta(days=self.refresh_days))
    ...
    def _encode(self, claims: dict[str, Any], token_type: str, expires: timedelta) -> str:
